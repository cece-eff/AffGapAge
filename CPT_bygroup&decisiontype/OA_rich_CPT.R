###OLDER ADULTS AFFECT-RICH CPT MODELING
...
choices <- oa_new_choices.se
prospectsA <- oa_new_gambles.3d.se[,1:6,]
prospectsB <- oa_new_gambles.3d.se[,7:12,]

#Determine cumulative probabilities
cumprobsA = cbind(oa_new_gambles.3d.se[,2,1], rowSums(oa_new_gambles.3d.se[,c(2,4),1]), rowSums(oa_new_gambles.3d.se[,c(2,4,6),1]), rowSums(oa_new_gambles.3d.se[,c(4,6),1]), oa_new_gambles.3d.se[,6,1])
cumprobsB = cbind(oa_new_gambles.3d.se[,8,1], rowSums(oa_new_gambles.3d.se[,c(8,10),1]), rowSums(oa_new_gambles.3d.se[,c(8,10,12),1]), rowSums(oa_new_gambles.3d.se[,c(10,12),1]), oa_new_gambles.3d.se[,12,1])
...
##affect rich
samples.oa.rich <- jags.parallel(data=data, inits=myinits, parameters=parameters,
                            model.file ="CPT_hierarchical_GEd_negative_reparam.txt", n.chains=nChains, n.iter=50000, 
                            n.burnin=2000, n.thin=2, DIC=T)

##save
save(samples.oa.rich, file='samples.oa.rich.RData')

##summarize and plot
samples.oa.rich$BUGSoutput$summary
print(samples.oa.rich, dig=3)
plot(samples.oa.rich)

#check effective samples
coda::effectiveSize(samples.oa.rich)

#all mean posterior distributions 
samples.oa.rich$BUGSoutput$mean


#create object with list of posterior distributions for all parameters
OA_rich<-samples.oa.rich$BUGSoutput$sims.list

##save list as file to reopen later
list.save(OA_rich, 'OA_rich.rds')

#density plots for posterior distributions of group level parameters
plot(density(OA_rich$mu.alpha))
plot(density(OA_rich$mu.delta.loss))
plot(density(OA_rich$mu.gamma))
plot(density(OA_rich$mu.theta))

#create MCMC samples from posterior distribution to run Kruschke code 
jagsfit_oa_rich.mcmc<-as.mcmc(samples.oa.rich)
...
diagMCMC(codaObject = jagsfit_oa_rich.mcmc, parName="mu.alpha")
diagMCMC(codaObject = jagsfit_oa_rich.mcmc, parName="mu.delta.loss")
diagMCMC(codaObject = jagsfit_oa_rich.mcmc, parName="mu.gamma")
diagMCMC(codaObject = jagsfit_oa_rich.mcmc, parName="mu.theta")

#plot the posterior distribution based on the JAGS output and 95% HDI
plotPost(jagsfit_oa_rich.mcmc[,"mu.alpha"], main="oa.rich.mu.alpha", xlab=bquote(alpha), cenTend = "mean")
plotPost(jagsfit_oa_rich.mcmc[,"mu.delta.loss"], main="oa.rich.mu.delta.loss", xlab=bquote(delta),  cenTend = "mean")
plotPost(jagsfit_oa_rich.mcmc[,"mu.gamma"], main="oa.rich.mu.gamma", xlab=bquote(gamma),  cenTend = "mean")
plotPost(jagsfit_oa_rich.mcmc[,"mu.theta"], main="oa.rich.mu.theta", xlab=bquote(theta),  cenTend = "mean")
